...
export async function GET(req: NextRequest) {
  ...
  // Parallel fetch: monitors alerts, CRM activity, recent briefs, notifications
  const [alertsRes, dealsRes, activitiesRes, briefsRes, notifRes] = await Promise.all([
    supabase.from('monitor_alerts').select('*').eq('user_id', user.id).gte('created_at', dayStart).order('created_at', { ascending: false }).limit(10),
    supabase.from('deals').select('*').eq('user_id', user.id).order('updated_at', { ascending: false }).limit(5),
    supabase.from('activities').select('*').eq('user_id', user.id).gte('created_at', dayStart).order('created_at', { ascending: false }).limit(10),
    supabase.from('research_briefs').select('id,topic,created_at').eq('user_id', user.id).order('created_at', { ascending: false }).limit(5),
